...
过去几十年，我们对软件的基本共识是：软件在发布之前完成主要设计和实现，用户通过预先定义好的界面使用它，变化通过需求、开发、测试、发布的周期进入系统。这条路径支撑了金融核心、电信设备、工业控制等所有严肃软件领域，它背后是一整套围绕确定性构建的工程体系。
...
在光谱左端，是**传统软件（Traditional Software）**：关键能力在发布前完成定义与实现，强调的是确定性、可预期、可审计。金融交易系统、数据库内核、系统软件，这些对正确性有极致要求的系统仍然依赖这种形态。它们不会消失，也消受不了 "让 Agent 临时改一下" 这种待遇。
...
知道了它是什么，下一个问题是：它凭什么成立？它和传统的 "可配置软件" 到底有什么本质不同？

## 柔性软件的特点

柔性软件之所以能成为一个独立的品类，而不是传统软件加一些动态配置的简单变体，根本原因是它背后的扩展逻辑变了。

传统软件的扩展性建立在 "设计者预判" 之上：架构师在软件设计阶段预见到可能的扩展方向，在这些方向上预留钩子和接口，用户通过填写配置项或编写插件来定制行为。这种模式在需求稳定时运作良好，但当变化方向和速度超出预判范围，预设的接口就成了限制。实际工程中常见的应对策略是不断增加配置项，把所有差异压成参数，最后配置系统复杂到没人能完整理解 —— 这就是 "配置爆炸"。
...
## 传统软件的柔性化：从 "能被用" 到 "能生长"

可能有人会觉得，柔性软件听起来不错，但自己的系统是一个已经跑了很多年的传统软件，难道要推倒重来？其实不需要。传统软件的柔性化是一条渐进路线，可以分步走，每一步都有独立的收益。

**第一步：让软件能被 Agent 使用。**

这是门槛最低的一步。传统软件的设计长期以来主要考虑 "如何让人使用"，界面设计、操作流程、错误提示都是围绕人类用户的认知特点展开的。在 Agent 时代，软件还需要考虑 "如何被 Agent 高效理解和使用"。这并不意味着要推到重写，而是在现有系统之上增加一层面向 Agent 的操作接口。
...
NanoClaw 是这个模式的典型范本：极小的核心代码 + Skills 驱动的扩展机制。用户说 "把触发词改成 @Bob"，Agent 改对应的源码文件；用户说 "每天早上 9 点给我发一份 AI 新闻简报"，Agent 通过 Skills 组合实现这个定时任务。整个过程不存在一个 "触发词配置项" 或 "定时任务配置面板" —— 行为的改变直接在源码层面发生。
...
这一步虽然还在路上，但方向是清晰的。Hermes Agent 已经展示了 "Agent 既是智能体又是软件平台" 的双重属性。随着端侧推理引擎越来越成熟，内置小型模型做特定领域推理的成本正在快速下降。未来可能会有越来越多的软件走向这个方向：把自己从一个工具变成一个能持续演进的领域智能体。

## 对软件工程的影响

柔性软件的出现，不只是给软件形态光谱增加了一个新品种，它对软件工程本身的影响更值得关注。
...
"柔性软件" 这个词听起来可能太温和了，不够颠覆。但回顾软件工程的历史，那些真正产生深远影响的变革，往往不是推翻一切的革命叙事，而是在确定性和灵活性之间找到新平衡点的实践。

传统软件工业花了几十年时间，建立起了一套围绕确定性运转的工程体系：需求分析、架构设计、开发测试、发布运维。这套体系不会消失，但它需要面对一个它从未认真考虑过的变量：软件在使用阶段仍然可以被塑造和改变。柔性软件不是什么玄妙的新东西，它只是坦诚地面对了这个问题，并且给出了一个务实的回应：把确定的交给工程，把变化的交给 Agent。
