...
去年，我作为技术顾问参与了华为昇腾 CANN 的 Ascend C 算子开发的易用性改进与评估项目。评估工作本身很直接：以第三方顾问视角端到端审视完整的算子开发流程，从工具链安装、查阅文档和示例代码、编写代码、构建测试到性能调优，然后分析所有体验不佳的地方，给出一份改进建议清单。
...
同时，评估 Agent 的角色也得到扩展：它不仅是度量易用性的工具，更是 **Ascend C Development Agent 的评估和优化工具**，可以持续进行对官方提供的所有开发界面（文档、示例、API、库、工程、工具...）包括开发 Agent 进行客观度量，识别改进空间，推动持续优化。
...
这些特性使得传统的 MLOps 方法不足以支撑 Agent 系统。正如 [《AgentOps: Enabling Observability of LLM（Large Language Model） Agents》](https://arxiv.org/abs/2411.05285) 这篇论文所指出的，Agent 系统需要一套专门的可观测性框架，用于 **系统化追踪 Agent 的执行轨迹和生命周期管理**。
...
在 Ascend C 开发 Agent 的案例中，这个过程遵循着如下的四步循环：
...
而与之对应的双 Agent 架构的强化范式是：

- **环境**：软件开发的环境配置和工具链。
- **状态**：任务的当前进展（搜索、编码、构建、测试）。
- **动作**：开发 Agent 的每一步决策（搜索、写代码、调工具）。
- **评价**：评估 Agent 给出的度量指标（Token、时间、成功率）。
- **权重更新**：优化开发 Agent 的 Rules/Skills 配置。
- **策略迭代**：改进后重新执行，验证优化效果。
...
这个 "在线强化" 的想法不是空想，实际上在当前的 Agent 工具中已经有了端倪。

我们看到 Anthropic 的 Claude Code 近来升级的几个关键能力：

- **完整的 session 记录**：每一次开发过程的完整日志，这个能力一直都有，一般在 `~/.claude/project` 目录下。
- **Skills 的热加载**：v2.1.0 版本开始支持，无需重启 Agent，动态加载更新后的 Skills 配置。
- **`/insights` 命令**：v2.1.30 版本开始支持，基于用户的对话/使用历史日志，生成诊断报告，给出改进建议。

这三个能力如果闭环打通，就几乎实现了在线强化：
...
社区已经有开发者开始探索这个方向。比如有人在 Reddit 上分享，通过分析 Claude Code 的日志，自动优化 Skills 配置，让 Agent 在特定任务上的表现提升。

### 这一范式的深远意义与适用边界

从离线强化学习到在线强化学习，不仅是技术路径的转变，更是 Agent 系统设计范式的根本升级。这一转变将带来几个层面的深远影响：

- **从 "昂贵训练" 到 "轻量调优"**：传统强化学习需要收集大量交互数据、进行耗时的梯度更新，而在线强化范式通过动态调整 Prompt、Rules 和 Skills 配置，实现了运行时低成本优化。这意味着一些 Agent 的改进不再依赖周期性的重新训练，而是可以像软件热更新一样实时生效，大大降低了迭代成本和延迟。
...
而在大量 **结构化、可度量、评估和改进规则相对明确** 的领域——如软件开发、流程合规性检查、教学效果评估、运维自动化等——在线强化范式能够发挥巨大作用。它使得 Agent 系统可以持续自我优化，不断降低错误率、提升效率、适应用户需求，最终实现 "越用越好用" 的良性循环。
...
正如前文所述，当 Agent 具备完整的可观测性，当评估可以自动化，当优化可以基于运行时数据实时调整，Agent 系统就具备了自我进化、自我适应的潜力。这种进化不再单一依赖离线训练和版本发布，而是在运行中持续学习、持续优化，甚至能够根据任务和环境的变化自主调整策略和配置。
